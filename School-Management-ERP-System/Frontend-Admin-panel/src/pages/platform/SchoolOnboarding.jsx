...
export default function SchoolOnboarding() {
  ...
  const createSchool = async () => {
    ...
    try {
      ...
      toast("School created — profile configured");
      setStepIndex(1);
    } catch (err) {
  ...
  const createAdmin = async () => {
    ...
    try {
      ...
      toast("School admin account created");
      setStepIndex(2);
    } catch (err) {
      toast(err.message, "error");
    } finally {
  ...
  const canContinue = (index) => index < stepIndex || !busy;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-slate-text/70">
                 onboarded its school admin and a live subscription. It will appear under Schools Management.
              </p>
            </div>
          </div>
          ...
          {STEPS.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex items-center gap-2 flex-1">
                <button
                  onClick={() => past && canContinue(index) && setStepIndex(index)}
      ...
      {stepIndex === 0 && (
        <Card title="Step 1 · School profile">
          <div className="grid sm:grid-cols-2 gap-3.5">
            ...
            <Input placeholder="Session (2026-27)" value={form.session} onChange={set("session")} />
            <Input placeholder="City" value={form.city} onChange={set("city")} />
            <Input placeholder="Phone" value={form.phone} onChange={set("phone")} />
            <Input type="email" placeholder="Contact email" value={form.email} onChange={set("email")} />
            <Input placeholder="Address" value={form.address} onChange={set("address")} className="sm:col-span-2" />
          </div>
          <div className="flex justify-end mt-5">
            <Button variant="amber" onClick={createSchool} disabled={busy}>
              <Plus size={15} /> {busy ? "Creating…" : "Create school"} <ArrowRight size={15} />
            </Button>
          </div>
        </Card>
      )}
      ...
      {stepIndex === 1 && (
        <Card title="Step 2 · School admin account">
          ...
          <div className="flex justify-between mt-5">
            ...
            <Button variant="amber" onClick={createAdmin} disabled={busy}>
              <Plus size={15} /> {busy ? "Creating…" : "Create admin"} <ArrowRight size={15} />
            </Button>
          </div>
        </Card>
      )}
      ...
      {stepIndex === 2 && (
        <Card title="Step 3 · Subscription plan">
          ...
          <div className="flex justify-between mt-5">
            ...
            <Button variant="amber" onClick={assignPlan} disabled={busy || !planId}>
              {busy ? "Assigning…" : "Assign plan"} <ArrowRight size={15} />
            </Button>
          </div>
        </Card>
      )}
        ...
        <Card title="Step 4 · Launch">
          <div className="space-y-2 text-[13.5px]">
            <p className="text-slate-text">
              <span className="font-semibold text-ink">{form.name || "School"}</span> is configured, the admin account is
              created, and the subscription is assigned. Marking it live completes onboarding and it enters the operator
              dashboard.
            </p>
            <p className="text-slate-text/70">
              Code: <span className="font-mono text-ink">{schoolCode}</span>
            </p>
          </div>
          <div className="flex justify-between mt-5">
