...
const FAQS = [
  {
    ...
    reply:
      "Hi! 👋 I'm the Brightwood ERP support assistant. Ask me about fees, attendance, timetable, exams, results, library, transport or login issues — or type 'help' to reach a human.",
  },
  {
    ...
    <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-3">
      {open && (
        <div className="w-[calc(100vw-2rem)] max-w-[360px] h-[480px] max-h-[72vh] bg-white rounded-2xl shadow-2xl border border-ink/10 overflow-hidden flex flex-col">
          ...
          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-2.5 bg-paper"
          >
            {messages.map((msg, i) =>
              msg.from === "bot" ? (
                <div
          ...
          {/* Quick chips */}
          <div className="px-3 pt-2 flex gap-1.5 overflow-x-auto scrollbar-thin shrink-0">
            {CHIPS.map((c) => (
              <button
                key={c}
                onClick={() => send(c)}
