...
function validateForm(form) {
  ...
  else if (childName.length < 2) errors.childName = "Child name must be at least 2 characters";
  else if (!NAME_RE.test(childName)) errors.childName = "Name can only contain letters, spaces and .-'";

  const parentName = form.parentName.trim();
  if (!parentName) errors.parentName = "Parent / guardian name is required";
  else if (parentName.length < 2) errors.parentName = "Parent name must be at least 2 characters";
  else if (!NAME_RE.test(parentName)) errors.parentName = "Name can only contain letters, spaces and .-'";

  if (!form.classApplied) errors.classApplied = "Select a class";
  ...
  if (form.status === "Admission Confirmed" && !form.admissionNo.trim()) {
    errors.admissionNo = "Admission ID is required when admission is confirmed";
  }
...
export default function AdmissionEnquiry() {
  ...
  const changeStatus = async (id, newStatus, inlineAdmissionNo) => {
    const item = list.find((e) => e.id === id);
    if (!item) return;

    if (newStatus === "Admission Confirmed" && !item.admissionNo && !inlineAdmissionNo) {
      if (id === selectedId) {
    ...
    const payload = { status: backendStatus[newStatus] || "New" };
    if (newStatus === "Admission Confirmed" && inlineAdmissionNo) {
      payload.admissionNo = inlineAdmissionNo.trim();
    }

    try {
      await api.admissions.update(id, payload);
      setList((prev) =>
        prev.map((e) =>
          e.id === id
            ? {
                ...e,
                status: newStatus,
                ...(payload.admissionNo ? { admissionNo: payload.admissionNo } : {}),
              }
            : e,
        ),
      );
  ...
  const handleConfirmAdmission = () => {
    if (!pendingAdmissionId.trim()) return;
    changeStatus(selectedId, "Admission Confirmed", pendingAdmissionId);
  };

  const updateStatusFromList = async (id, newStatus) => {
    setUpdatingStatusId(id);
    const item = list.find((e) => e.id === id);
    if (!item) return;

    if (newStatus === "Admission Confirmed" && !item.admissionNo) {
      setSelectedId(id);
      setPendingAdmissionId("");
      setShowAdmissionInput(true);
                ...
                {showAdmissionInput && selected.status !== "Admission Confirmed" && (
                  <div className="mt-3 rounded-xl border border-amber/30 bg-amber/5 p-3 space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setPendingAdmissionId(e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") handleConfirmAdmission();
                      }}
                    />
                    <p className="text-[11px] text-slate-text/60">
                      Required to confirm admission — links enquiry to student record.
                    </p>
                    <div className="flex gap-2 pt-1">
                      ...
                      >
                        <BadgeCheck size={13} /> Confirm Admission
                      </Button>
                      <Button
                        variant="outline"
                        className="!py-1.5"
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[92vh] flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    className={formErrors.childName ? "ring-2 ring-alert/40 border-alert" : ""}
                    aria-invalid={Boolean(formErrors.childName)}
                  />
                  {formErrors.childName && (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.childName}</p>
                  )}
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    className={formErrors.parentName ? "ring-2 ring-alert/40 border-alert" : ""}
                    aria-invalid={Boolean(formErrors.parentName)}
                  />
                  {formErrors.parentName && (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.parentName}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <SearchableSelect
                    ...
                    className={formErrors.classApplied ? "ring-2 ring-alert/40 rounded-xl" : ""}
                  />
                  {formErrors.classApplied && (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.classApplied}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={formErrors.contact ? "ring-2 ring-alert/40 border-alert" : ""}
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={formErrors.date ? "ring-2 ring-alert/40 border-alert" : ""}
                    aria-invalid={Boolean(formErrors.date)}
                  />
                  {formErrors.date && (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.date}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={formErrors.followUp ? "ring-2 ring-alert/40 border-alert" : ""}
                    aria-invalid={Boolean(formErrors.followUp)}
                  />
                  {formErrors.followUp && (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.followUp}</p>
                  )}
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    className={formErrors.admissionNo ? "ring-2 ring-alert/40 border-alert" : ""}
                    aria-invalid={Boolean(formErrors.admissionNo)}
                  />
                  {formErrors.admissionNo ? (
                    <p className="text-alert text-[11.5px] mt-1">{formErrors.admissionNo}</p>
                  ) : (
            ...
            <div className="px-6 py-4 border-t border-black/[0.06] flex justify-end gap-2 shrink-0">
              ...
              <Button
                ...
                disabled={saving}
              >
                <Save size={15} /> {saving ? "Saving..." : editId ? "Update" : "Save"} Enquiry
              </Button>
            </div>
          </div>
        </div>
