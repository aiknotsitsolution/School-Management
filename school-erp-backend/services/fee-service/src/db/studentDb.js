...
// student-service database (erp_student). So that bulk invoice generation
// cannot fabricate invoices for arbitrary studentIds, this module opens a
...
//
// It is only ever loaded inside bulk-generation paths, so read-only flows carry
...
//
// NOTE: fee-service, academic-service and auth-service each load Student
// mirrors. They share a schema shape but are independent module
...
// Lazily builds and connects the mirror model. The first bulk-generation
// request pays the connect cost; every later call reuses the same model.
