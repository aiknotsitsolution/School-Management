...
const subscriptionPaid = async (req, res) => {
  try {
    const { schoolId, planId, amount, currency, orderId, providerOrderId } = req.body || {};
    if (!schoolId || !planId) {
    ...
    if (Number(plan.price) !== Number(amount)) {
      return res.status(400).json({ success: false, message: "Paid amount does not match the plan price" });
    }
    ...
    const next = await Subscription.create({
      ...
      metadata: { paymentOrderId: orderId || null, providerOrderId: providerOrderId || null },
    });
